...
def generate_waiting_time(df):
    '''Generate synthetic waiting time based on traffic conditions'''
    df['queue_density'] = df['traffic_volume'] / (df['avg_vehicle_speed'] + 1e-5)
    signal_multiplier = {'Red': 1.0, 'Yellow': 0.3, 'Green': 0.05}
    df['signal_multiplier'] = df['signal_status'].map(signal_multiplier)
    weather_factor = {'Sunny': 1.0, 'Cloudy': 1.1, 'Windy': 1.15, 'Rainy': 1.3, 'Foggy': 1.25}
    df['weather_factor'] = df['weather_condition'].map(weather_factor)
    total_vehicles = df['vehicle_count_cars'] + df['vehicle_count_trucks'] + df['vehicle_count_bikes']
    df['density_factor'] = 1 + (total_vehicles / total_vehicles.max())
    df['truck_ratio'] = df['vehicle_count_trucks'] / (total_vehicles + 1e-5)
    df['truck_penalty'] = 1 + (df['truck_ratio'] * 0.5)
    df['accident_multiplier'] = np.where(df['accident_reported'] == 1, 2.0, 1.0)
    df['hour'] = pd.to_datetime(df['timestamp']).dt.hour
    df['peak_factor'] = np.where(
        ((df['hour'] >= 7) & (df['hour'] <= 10)) | ((df['hour'] >= 17) & (df['hour'] <= 20)),
        1.4, 1.0
    )
    base_waiting_time = df['queue_density'] * df['signal_multiplier'] * df['density_factor'] * 30
    waiting_time = (base_waiting_time * df['weather_factor'] * df['truck_penalty'] * 
                   df['accident_multiplier'] * df['peak_factor'])
    noise = np.random.normal(1.0, 0.1, size=len(df))
    waiting_time = waiting_time * noise
    waiting_time = np.clip(waiting_time, 0, 300)
    df['waiting_time_seconds'] = waiting_time.round(2)
    df['avg_waiting_per_vehicle'] = (df['waiting_time_seconds'] / (total_vehicles + 1)).round(2)
    cols_to_drop = ['queue_density', 'signal_multiplier', 'weather_factor', 
                    'density_factor', 'truck_ratio', 'truck_penalty', 
                    'accident_multiplier', 'peak_factor', 'hour']
...
def preprocess_traffic_data(input_file='smart_traffic_management_dataset.csv'):
    ...
    df = generate_waiting_time(df)
    print(f"  Added columns: waiting_time_seconds, avg_waiting_per_vehicle")
    print(f"  Waiting time range: {df['waiting_time_seconds'].min():.2f}s - {df['waiting_time_seconds'].max():.2f}s")
    ...
    numerical_features = [
        ...
        'waiting_time_seconds', 'avg_waiting_per_vehicle'  
    ]
